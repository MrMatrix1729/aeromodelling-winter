...
void loop() {
  ...
  // Calculate new altitude and calls a smoothning function with previous altitude and 
...
double correctedAngle(int gyro, int accX, int accY, int accZ, int axis) {
  ...
  double accAngleX = (atan(accY / sqrt(pow(accX, 2) + pow(accZ, 2))) * 180 / PI) - accXError; 
  double accAngleY = (atan(-1 * accX / sqrt(pow(accY, 2) + pow(accZ, 2))) * 180 / PI) + accYError; //correct the errors
...
double ema(double oldValue, double newValue) {
  ...
  double Value = (1.0 - smoothingFactor) * oldValue + smoothingFactor * newValue;
  return Value;
}
